import type { SourceCheckResult } from '../core/source-check.types';
...
// Helper: normalize state to uppercase 2-letter if already such, otherwise uppercase anyway
function normState(value?: string | null): string {
  if (!value) return '';
  return String(value).trim().toUpperCase();
}

// Helper: ensure zip5 is 5-digit if present
function normZip5(value?: string | null): string {
  if (!value) return '';
...
export function mapSourceToValidated(result: SourceCheckResult, hint: ValidatedAddressDto): ValidatedAddressDto {
  ...
  const state = normState(candidate.state_code || candidate.state || base.state);
  ...
  const zipCode = normZip5(postcode) || base.zipCode;
